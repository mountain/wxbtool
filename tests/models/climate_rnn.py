...
class TestDataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        for var in setting.vars:
            inputs.update({var: np.zeros((setting.input_span, 32, 64), dtype=np.float32)})
            targets.update({var: np.zeros((setting.pred_span, 32, 64), dtype=np.float32)})
...
class TestModel(SpecRNN):
    ...
    def forward(self, **kwargs):
        inputs = kwargs['data']
        assert inputs.dim() == 5, "Input data must be 5D tensor (batch_size, channels, time_steps, height, width)"
